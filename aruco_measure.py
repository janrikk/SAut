...
class EKFSLAMNode:
    def __init__(self):
        ...
        if rospy.has_param('~initial_pose'):
            ...
            rospy.loginfo(f"[INIT_PARAM] Pose inicial: x={x0:.2f}, y={y0:.2f}, θ={np.degrees(th0):.1f}°")
        else:
            self.state = np.array([0.0, 0.0, 0.0])
            self.cov = np.eye(3) * 1e-3
            self.initialized_from_aruco = False

        self.odom_state = self.state[:3].copy()  # ESTADO PARA ODOMETRIA PURA

        self.Q = np.diag([0.002, 0.002, np.deg2rad(0.3)])**2
    ...
    def ekf_predict(self, u, dt):
        ...
        # Atualiza estado odométrico (independente do EKF)
        self.odom_state = motion_model(self.odom_state, u, dt)

        # Publicar odom_path com base apenas na odometria
        ...
        rospy.loginfo(f"[PREDICT] u={u}, dt={dt:.3f}, pose={self.state[:3]}")
    ...
    def aruco_callback(self, msg):
        ...
        if mahalanobis > threshold:
            rospy.logwarn(f"[REJEITADO] ArUco {landmark_id} → Mahalanobis={mahalanobis:.2f} > {threshold}")
            return
        ...
        rospy.loginfo(f"[UPDATE] ArUco {landmark_id} (obs={obs_count}) → Pose = {self.state[:3]}")
        self.publish_pose()
        self.publish_landmarks()
